...
from tensorflow.keras.layers import Conv3D, MaxPool3D, AveragePooling3D
...
def _shortcut_spc(input, residual):
    ...
    if stride_dim1 > 1 or stride_dim2 > 1 or stride_dim3 > 1 or not equal_channels:
        shortcut = Convolution3D(nb_filter=residual.shape[CHANNEL_AXIS],
...
def _shortcut(input, residual):
    ...
    shortcut = Conv3D(kernel_initializer="he_normal", strides=(stride_dim1, stride_dim2, stride_dim3), kernel_regularizer=regularizers.l2(0.0001),
                          filters=residual.shape[CHANNEL_AXIS], kernel_size=(1, 1, 1), padding='valid')(input)
...
class ResnetBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn_spc, block_fn, repetitions1, repetitions2):
        ...
        conv_spc_results = _conv_bn_relu_spc(nb_filter=128,kernel_dim1=1,kernel_dim2=1,kernel_dim3=block_output_spc.shape[CONV_DIM3])(block_output_spc)

        # print("conv_spc_result shape:", conv_spc_results.shape)

        conv2_spc = Reshape((conv_spc_results.shape[CONV_DIM1],conv_spc_results.shape[CONV_DIM2],conv_spc_results.shape[CHANNEL_AXIS],1))(conv_spc_results)
        ...
        # Classifier block
        pool2 = AveragePooling3D(pool_size=(block.shape[CONV_DIM1],
                                            block.shape[CONV_DIM2],
                                            block.shape[CONV_DIM3],),
